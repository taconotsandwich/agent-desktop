...
pub struct AtspiConnection {
    inner: Mutex<Option<Arc<AccessibilityConnection>>>,
}
...
impl AtspiConnection {
    ...
    pub async fn get(&self) -> Result<Arc<AccessibilityConnection>, BackendError> {
        let mut inner = self.inner.lock().await;
        if let Some(connection) = inner.as_ref() {
            let healthy = match zbus::fdo::DBusProxy::new(connection.connection()).await {
                Ok(proxy) => proxy.list_names().await.is_ok(),
                Err(_) => false,
            };
            if healthy {
        ...
        let connection = Arc::new(AccessibilityConnection::new().await.map_err(|error| {
            BackendError::BusDisconnected {
                detail: format!("atspi connect: {error}"),
            }
        })?);
...
pub async fn walk(
    ...
    while let Some((proxy, depth, parent_ref)) = queue.pop_front() {
        ...
        if !visited.insert(payload.clone()) {
            continue;
        }
        if visited.len() > opts.max_elements.saturating_mul(4) {
            truncated = true;
            break;
        }
        let id = Ref(payload);
        ...
        if depth > 0 && !states_vec.iter().any(|state| state == "showing") {
            continue;
        }
        ...
        if keep {
            out.push(FlatElement {
                id: id.clone(),
                bus_guid: conn.connection().server_guid().to_string(),
        ...
        for i in 0..child_count.min(opts.max_elements as u32) as i32 {
            if queue.len() >= opts.max_elements.saturating_mul(4) {
                truncated = true;
                break;
            }
...
async fn element_peripherals(
    ...
    let value: Option<TrustedText> = match proxies.text().await {
        Ok(t) => match t.character_count().await {
            Ok(n) if n > 0 => match t.get_text(0, n.min(4096)).await {
