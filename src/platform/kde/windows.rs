...
use crate::platform::drivers::{Bbox, Probe, Ref, WindowDriver, WindowInfo};
...
impl WindowDriver for KwinWindows {
    ...
    async fn query(&self) -> Result<Vec<WindowInfo>, ToolError> {
        let body = r#"
            ...
            for (var i = 0; i < windows.length; i++) {
                ...
                out.push({
                    ...
                    appId: w.desktopFileName || "",
                    clientProtocol: typeof w.waylandClient === "boolean"
                        ? (w.waylandClient ? "wayland" : "x11") : null,
                    pid: w.pid || 0,
        ...
        let root: serde_json::Value = serde_json::from_str(&payload).map_err(|e| {
    ...
    items
        ...
        .filter_map(|v| {
            ...
            Some(WindowInfo {
                ...
                app_id: v.get("appId").and_then(|v| v.as_str()).unwrap_or("").into(),
                pid: v
                    .get("pid")
                    .and_then(|v| v.as_u64())
...
impl KwinWindows {
    async fn mutate(&self, id: &Ref, action: &str, geo: Option<Bbox>) -> Result<(), ToolError> {
        ...
        let op = match action {
            "focus" => "list[i].minimized = false; ws.activeWindow = list[i];".to_string(),
            "minimize" => "list[i].minimized = true;".to_string(),
            "maximize" => "list[i].setMaximize(true, true);".to_string(),
            "restore" => {
                "list[i].minimized = false; list[i].setMaximize(false, false);".to_string()
            }
