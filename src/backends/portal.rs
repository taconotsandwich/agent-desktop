...
//! presence, not authorization — `doctor` surfaces that honestly.
...
async fn await_response(
    ...
    let msg = tokio::time::timeout(std::time::Duration::from_secs(60), stream.next())
        .await
    ...
    let (code, results): (u32, HashMap<String, OwnedValue>) =
        msg.body().deserialize().map_err(|e| BackendError::ExternalCommandFailed {
    ...
    async fn capture(&self, target: ShotTarget, max_long_edge: u32) -> Result<Shot, ToolError> {
        if !matches!(target, ShotTarget::Full) {
            return Err(BackendError::Unsupported {
                reason: "portal targeted capture lands later; use full".into(),
            }
            .tool(false));
        }
...
async fn capture_full(bus: &Connection, max_long_edge: u32) -> Result<Shot, BackendError> {
    ...
    let handle: OwnedObjectPath = format!("/org/freedesktop/portal/desktop/request/{sender}/{token}")
        .try_into()
    ...
    let _ = handle;
    let (code, results) = await_response(bus, req).await?;
    if code != 0 {
...
/// Open a RemoteDesktop session and return the EIS fd. First call may show
/// the portal's share dialog; the response waits up to 60s.
pub async fn remote_desktop_eis_fd(bus: &Connection) -> Result<std::os::fd::OwnedFd, BackendError> {
    let session = ensure_session(bus).await?;
    connect_eis_fd(bus, &session).await
}
...
pub async fn ensure_session(bus: &Connection) -> Result<OwnedObjectPath, BackendError> {
    ...
    let (code, _) = await_response(bus, req).await?;
    if code != 0 {
        return Err(BackendError::ExternalCommandFailed {
