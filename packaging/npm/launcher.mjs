...
if (process.platform !== "linux" || process.arch !== "x64") {
  console.error(
    "agent-desktop supports Linux x64 with glibc. Run it on your Linux desktop host.",
  );
  process.exit(1);
}
let packaged;
try {
  packaged = join(
    dirname(
      require.resolve("@taconotsandwich/agent-desktop-linux-x64/package.json"),
    ),
    "bin/agent-desktop",
  );
} catch {
  console.error(
    "The Linux binary package is missing. Reinstall with optional dependencies enabled.",
  );
  process.exit(1);
}
